...
export const checkPrice = async (symbol, st) => {
  const res = await fetch(`${st.URL}/ticker/price?symbol=${symbol}`, {
    method: "GET",
  });
  const json = await res.json();
  // console.log(json)
...
export const avgPrice = async (symbol, st) => {
  const res = await fetch(
    `${st.URL}/klines?symbol=${symbol}&interval=5m&limit=5`,
    { method: "GET" }
  );
  // console.log(res)
  const json = await res.json();
...
export const avgPrice30 = async (symbol, st) => {
  const res = await fetch(
    `${st.URL}/klines?symbol=${symbol}&interval=1m&limit=30`,
    { method: "GET" }
  );
  // console.log(res)
  const json = await res.json();
...
export const parseExchangeInfo = async (json,wantedMarkets) => {
  ...
  json.symbols.forEach((market) => {
    ...
    if (wantedMarkets.indexOf(market.symbol) != -1) {
      ...
      info[market.symbol].quoteAsset = market.quoteAsset;
      info[market.symbol].baseAsset = market.baseAsset;

      // obtain filter values from the exchange
      market.filters.forEach((filter) => {
        if (filter.filterType == "LOT_SIZE") {
          if (filter.minQty >= `1.00`) {
            info[market.symbol].minQty = Math.floor(Math.log10(filter.minQty));
          } else {
            info[market.symbol].minQty = -Math.floor(Math.log10(filter.minQty));
          }
        }
        ...
        if (filter.filterType == "PRICE_FILTER") {
          if (filter.minPrice >= `1.00`) {
            info[market.symbol].quoteAssetPrecision = Math.floor(
              Math.log10(filter.minPrice)
            );
          } else {
            info[market.symbol].quoteAssetPrecision = -Math.floor(
              Math.log10(filter.minPrice)
        ...
        if (filter.filterType == "MIN_NOTIONAL") {
          info[market.symbol].minOrder = filter.minNotional
            ? filter.minNotional
            : filter.notional;
        }
      });
